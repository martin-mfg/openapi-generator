...
class ExampleApiController(@Autowired(required = true) val service: ExampleApiService) {
    ...
    @ApiOperation(
        ...
        response = java.math.BigDecimal::class,
        responseContainer = "List")
    @ApiResponses(
        value = [ApiResponse(code = 200, message = "The response with results", response = java.math.BigDecimal::class, responseContainer = "List")])
    @RequestMapping(
        method = [RequestMethod.GET],
        value = ["/example/someMethod"],
        produces = ["application/json"]
    )
    fun retrieveSomething(): ResponseEntity<List<java.math.BigDecimal>> {
