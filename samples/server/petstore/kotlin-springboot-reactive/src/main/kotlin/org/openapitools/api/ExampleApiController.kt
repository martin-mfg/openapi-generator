...
class ExampleApiController(@Autowired(required = true) val service: ExampleApiService) {
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "200", description = "The response with results", content = [Content(schema = Schema(implementation = java.math.BigDecimal::class))]) ]
    )
    @RequestMapping(
    ...
    fun retrieveSomething(): ResponseEntity<Flow<java.math.BigDecimal>> {
