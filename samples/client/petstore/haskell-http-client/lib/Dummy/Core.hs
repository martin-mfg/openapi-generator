...
-- | display the config
instance P.Show DummyConfig where
  show c =
    T.printf
...
-- | constructs a default DummyConfig
--
-- configHost:
--
-- @http://localhost@
...
-- @"dummy/0.1.0.0"@
--
newConfig :: IO DummyConfig
newConfig = do
    logCxt <- initLogContext
    return $ DummyConfig
        { configHost = "http://localhost"
        , configUserAgent = "dummy/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => DummyConfig -> auth -> DummyConfig
addAuthMethod config@DummyConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: DummyConfig -> IO DummyConfig
...
withStderrLogging :: DummyConfig -> IO DummyConfig
...
withNoLogging :: DummyConfig -> DummyConfig
...
data DummyRequest req contentType res accept = DummyRequest
  { rMethod  :: NH.Method   -- ^ Method of DummyRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of DummyRequest
  , rParams   :: Params -- ^ params of DummyRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (DummyRequest req contentType res accept) NH.Method
rMethodL f DummyRequest{..} = (\rMethod -> DummyRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (DummyRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f DummyRequest{..} = (\rUrlPath -> DummyRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (DummyRequest req contentType res accept) Params
rParamsL f DummyRequest{..} = (\rParams -> DummyRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (DummyRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f DummyRequest{..} = (\rAuthTypes -> DummyRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => DummyRequest req contentType res accept -> param -> DummyRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: DummyRequest req contentType res accept -> param -> DummyRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: DummyRequest req contentType res accept -> param -> DummyRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method
          ...
          -> DummyRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = DummyRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     DummyRequest req contentType res accept
  -> [NH.Header]
  -> DummyRequest req contentType res accept
...
removeHeader :: DummyRequest req contentType res accept -> [NH.HeaderName] -> DummyRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => DummyRequest req contentType res accept -> DummyRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => DummyRequest req contentType res accept -> DummyRequest req contentType res accept
...
addForm :: DummyRequest req contentType res accept -> WH.Form -> DummyRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: DummyRequest req contentType res accept -> NH.Part -> DummyRequest req contentType res accept
...
_setBodyBS :: DummyRequest req contentType res accept -> B.ByteString -> DummyRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: DummyRequest req contentType res accept -> BL.ByteString -> DummyRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => DummyRequest req contentType res accept -> P.Proxy authMethod -> DummyRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> DummyRequest req contentType res accept
    -> IO (DummyRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> DummyConfig
  -> IO (DummyRequest req contentType res accept)
_applyAuthMethods req config@(DummyConfig {configAuthMethods = as}) =
  foldlM go req as
  where
