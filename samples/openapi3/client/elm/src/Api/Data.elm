...
type alias ExampleResponse =
    { myOnlyProperty : Maybe ( List Float )
    }
...
encodeExampleResponsePairs model =
    let
        pairs =
            [ maybeEncode "myOnlyProperty" (Json.Encode.list Json.Encode.float) model.myOnlyProperty
            ]
    in
    pairs
...
exampleResponseDecoder =
    Json.Decode.succeed ExampleResponse
        |> maybeDecode "myOnlyProperty" (Json.Decode.list Json.Decode.float) Nothing
